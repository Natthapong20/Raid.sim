...
class RAIDSimulator:
    ...
    def simulate_failure_and_recovery(self, failed_indices):
        ...
        if self.raid_level == '10':
            if any(idx >= self.num_disks for idx in failed_indices):
                raise ValueError("Invalid disk index for RAID 10.")
            if any(abs(idx1 - idx2) == self.num_disks // 2 for idx1 in failed_indices for idx2 in failed_indices if idx1 != idx2):
                raise ValueError("RAID 10 cannot recover if both disks in the same mirrored pair fail.")
