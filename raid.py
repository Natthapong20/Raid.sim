...
class RAIDSimulator:
    ...
    def calculate_parity(self, blocks):
        ...
        for block in blocks[1:]:
            parity ^= ord(block)
        return hex(parity)[2:].upper().zfill(2)  # แสดงเป็น HEX 2 หลัก
        ...
        if self.raid_level in {'5', '6'}:
            ...
            for block_index in range(len(failed_data)):
                ...
                for disk_index in range(self.num_disks):
                    ...
                    # เช็คว่าค่า parity เป็น HEX และไม่ใช่ '_' หรือ 'X'
                    recovered_block ^= int(block_value, 16) if block_value not in {'X', '_'} else 0
