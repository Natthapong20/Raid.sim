# RAID Simulator Pro Edition v2.1
# - Fixed Recovery Summary Disappear Issue
...
class RAIDGUI:
    def __init__(self, master):
        self.master = master
        master.title("RAID Simulator Pro Edition v2.1")
        ...
        tk.Button(self.input_frame, text="Save Log", command=self.save_log, bg="#FF9800", fg="white").pack(pady=5)
    ...
    def recover_disk(self):
        try:
            ...
            self.master.after(100, lambda: self.display_recovery_summary(index, recovered_data))
        except Exception as e:
            messagebox.showerror("Error", str(e))

    def display_recovery_summary(self, index, recovered_data):
        self.update_display()
        if recovered_data:
    ...
    def save_log(self):
        log = self.output_text.get(1.0, tk.END)
